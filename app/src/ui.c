...
static uint8_t fault_blink_count(uint32_t fault_mask)
{
    const uint32_t hardware_mask =
        AURORA_FAULT_FAST_MOS_OCP | AURORA_FAULT_FAST_BREAK | AURORA_FAULT_BUS_OVERVOLT |
        AURORA_FAULT_MOS_OVERTEMP | AURORA_FAULT_AMB_TEMP | AURORA_FAULT_MOS_NTC_OPEN |
        AURORA_FAULT_MOS_NTC_SHORT | AURORA_FAULT_AMB_NTC_OPEN | AURORA_FAULT_AMB_NTC_SHORT |
        ...
        AURORA_FAULT_PV_CURRENT_PLAUSIBILITY | AURORA_FAULT_BUS_ADC_SATURATION |
        AURORA_FAULT_DEMO_OUTPUT;
    const uint32_t battery_mask =
        AURORA_FAULT_BAT_UNDERVOLT | AURORA_FAULT_BAT_OVERVOLT | AURORA_FAULT_SETTINGS;
    const uint32_t pv_mask = AURORA_FAULT_FAST_PV_OCP | AURORA_FAULT_PV_UNDERVOLT |
                             AURORA_FAULT_PV_OVERVOLT | AURORA_FAULT_PV_OVERCURRENT |
    ...
    // 新增故障若尚未分类，仍必须显示3次硬件故障组闪，不能让FAULT灯熄灭。
    return (fault_mask != 0U) ? 3U : 0U;
}
 ...
 *               uint32_t elapsed_ms)
 * Input       : ctx - UI上下文；power_state - 功率级状态；
 *               fault_mask - 故障并集；elapsed_ms - 推进时间
 * Output      : RUN/FAULT两路逻辑点亮命令
 * Description : RUN在正常充电时闪烁、系统工作非充电时常亮；
 *               FAULT按1/2/3次组闪显示PV/电池/硬件类。
 *---------------------------------------------------------------------------*/
aurora_ui_output_t aurora_ui_step(aurora_ui_ctx_t *ctx, aurora_power_state_t power_state,
                                  uint32_t fault_mask, uint32_t elapsed_ms)
{
    ...
    if (blink_count != 0U)
    {
        const uint32_t pulse_window_ms = (uint32_t)blink_count * (2U * AURORA_UI_FAULT_HALF_MS);
        ...
        {
            output.led_fault_on =
                ((phase_ms % (2U * AURORA_UI_FAULT_HALF_MS)) < AURORA_UI_FAULT_HALF_MS);
        }
        return output;
    }
    ...
    {
        output.led_run_on = ((ctx->led_phase_ms % (2U * AURORA_UI_RUN_BLINK_HALF_MS)) <
                             AURORA_UI_RUN_BLINK_HALF_MS);
    }
    else if ((power_state != AURORA_POWER_OFF) && (power_state != AURORA_POWER_NO_SUN))
    {
