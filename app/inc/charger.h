...
typedef struct
{
    ...
    uint32_t charge_start_ms;                        /* 兼容调试：本轮状态机开始时间，不用于15h墙上计时。 */
    ...
    uint32_t active_charge_ms;                       /* 本轮真实功率传输累计时间，ms。 */
    uint32_t active_float_ms;                        /* Float阶段真实功率传输累计时间，ms。 */
    ...
    uint8_t state_reserved[2];                       /* 显式补齐ARM短枚举和布尔字段。 */
...
void aurora_charger_init(aurora_charger_ctx_t *ctx,
                         ...
                         aurora_battery_pack_t pack,
                         uint32_t now_ms);
void aurora_charger_account_active_time(aurora_charger_ctx_t *ctx,
