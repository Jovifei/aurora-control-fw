...
/*---------------------------------------------------------------------------*
 * Name        : static void set_voltage_calibration(drv_board_adc_calibration_t *calibration, int32_t divider_num, int32_t divider_den)
...
static void set_voltage_calibration(drv_board_adc_calibration_t *calibration,
                                    ...
                                    int32_t divider_den)
{
    calibration->kind = DRV_ADC_CALIBRATION_LINEAR;
...
/*---------------------------------------------------------------------------*
 * Name        : static void set_mos_ntc_calibration(drv_board_adc_calibration_t *calibration)
 * Input       : calibration - 标定输出
 * Output      : 无
 * Description : 写入原理图板载MOS NTC的上拉、R25、Beta和有效温度范围参数。
 *---------------------------------------------------------------------------*/
static void set_mos_ntc_calibration(drv_board_adc_calibration_t *calibration)
{
    calibration->kind = DRV_ADC_CALIBRATION_NTC_BETA;
    calibration->valid = true;
...
/*---------------------------------------------------------------------------*
 * Name        : bool drv_board_get_adc_calibration(size_t channel, drv_board_adc_calibration_t *calibration)
 ...
 * Description : 返回链接脚本保留的A页地址，供应用层读取和编程。
 *---------------------------------------------------------------------------*/
 ...
 * Description : 返回链接脚本保留的B页地址，供应用层读取和编程。
 *---------------------------------------------------------------------------*/
